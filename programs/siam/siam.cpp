...
nda::vector<double> siam_allfuncs(double beta, double u, double lambda, double eps, int niomtst, int nbos_tst, bool reduced, bool compressbasis,
  ...
  auto datafile = "../../../siam_data/SIAM_beta20_U5.0_ED_extracted_final.h5"; // Filename
                                                                               // for
                                                                               // Green's
                                                                               // function,
                                                                               // correlator,
  ...
  int eta_nmax = (eta_s_data.size() - 1) / 2; // Imag freq grid on n = -eta_nmax:eta_nmax
  ...
  int chi_nmax = chi_s_data.shape(0) / 2; // Imag freq grid on n = -chi_nmax:chi_nmax-1
  ...
  for (int k = 0; k < niom; ++k) {
    ...
    chi_d(k) = chi_d_data(chi_nmax + dlr2d_if_ph(k, 0), chi_nmax + dlr2d_if_ph(k, 1));
    chi_m(k) = chi_m_data(chi_nmax + dlr2d_if_ph(k, 0), chi_nmax + dlr2d_if_ph(k, 1));
    lam_s(k) = lam_s_data(chi_nmax + dlr2d_if(k, 0), chi_nmax + dlr2d_if(k, 1)) - 1;
    lam_d(k) = lam_d_data(chi_nmax + dlr2d_if_ph(k, 0), chi_nmax + dlr2d_if_ph(k, 1)) - 1;
    lam_m(k) = lam_m_data(chi_nmax + dlr2d_if_ph(k, 0), chi_nmax + dlr2d_if_ph(k, 1)) - 1;
  }
  ...
  if (!compressbasis) {
    ...
    std::tie(chi_s_c, chi_s_csing) = uncompress_basis(r, dlr2d_rfidx, chi_s_c_compressed);
    std::tie(chi_d_c, chi_d_csing) = uncompress_basis(r, dlr2d_rfidx, chi_d_c_compressed);
    std::tie(chi_m_c, chi_m_csing) = uncompress_basis(r, dlr2d_rfidx, chi_m_c_compressed);
    std::tie(lam_s_c, lam_s_csing) = uncompress_basis(r, dlr2d_rfidx, lam_s_c_compressed);
    std::tie(lam_d_c, lam_d_csing) = uncompress_basis(r, dlr2d_rfidx, lam_d_c_compressed);
    std::tie(lam_m_c, lam_m_csing) = uncompress_basis(r, dlr2d_rfidx, lam_m_c_compressed);
  }
...
nda::vector<double> siam_allfuncs_3term(double beta, double u, double lambda, double eps, int niomtst, int nbos_tst) {
  ...
  // pol_s += polarization_const(beta, itops, ifops_bos, gc, gc);
  auto pol_s = polarization_3term(beta, lambda, eps, itops, ifops_fer, ifops_bos, gc, gc, lam_s_c, lam_s_csing);
  pol_s *= -1.0 / 2;
  ...
  //     lam_d_csing);
  // pol_d += polarization_const(beta, itops, ifops_bos, grc, gc);
  auto pol_d = polarization_3term(beta, lambda, eps, itops, ifops_fer, ifops_bos, grc, gc, lam_d_c, lam_d_csing);

  // auto pol_m =
  //     polarization(beta, ifops_fer, ifops_bos, grc, gc, lam_m_c,
  //     lam_m_csing);
  // pol_m += polarization_const(beta, itops, ifops_bos, grc, gc);
  auto pol_m = polarization_3term(beta, lambda, eps, itops, ifops_fer, ifops_bos, grc, gc, lam_m_c, lam_m_csing);
