...
int main() {
  ...
  for (int i = 0; i < nexp; ++i) {
    ...
    lambda = beta;
    if (threeterm) {
      results(nda::range::all, i) = hubatom_allfuncs_3term(beta, u, lambda, eps, niomtst, nbos_tst);
    } else {
      results(nda::range::all, i) = hubatom_allfuncs(beta, u, lambda, eps, niomtst, nbos_tst, reduced, compressbasis, niom_dense);
    }
  }
  ...
  f.precision(16);
  for (int i = 0; i < nexp; ++i) {
    for (int j = 0; j < nresult; ++j) { f << results(j, i) << "\n"; }
  }
}